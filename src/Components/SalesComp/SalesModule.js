...
const SalesModule = ({
  ...
  console.log("editeDataediteData", editeData);
  ...
  const [isBatchProduction, setisBatchProduction] = useState(true);
  ...
  const AssemblySalesResult = new URLSearchParams(location.search).get(
    "AssemblySalesResult"
  );
  console.log(
    "AssemblySalesResultAssemblySalesResultAssemblySalesResult",
    AssemblySalesResult
  );

//   useEffect(() => {
//     const storedData = JSON.parse(sessionStorage.getItem('orderPlan')) || [];
//     setPartData(storedData);
//   }, []);
  ...
  useEffect(() => {
    if (open) {
      ...
      if (editeData.length === 0) {
        SaleGetOrderNo(
          handleSucessSaleGetOrderNo,
          handleExceptionSaleGetOrderNo
        );
  ...
  const handleSucessSaleGetOrderNo = (dataObject) => {
    ...
    if (isRejected) {
      SaleScrapRejectedItems(
        { saleId: dataObject?.id, scrapItems: selectedItems },
        handleScrapItemSuccess,
        handleScrapItemException
      );
    }
  };
  ...
  const LoadData = () => {
    ...
    // setisBatchProduction(editeData?.isBatchProduction || true);
    ...
    SaleOrderDataShow(
      ...
      handleSaleOrderDataShowSuccess,
      handleSaleOrderDataShowException
    );
  };
  ...
  const handleSubmit = (e) => {
    ...
    if (isAddButton) {
      SaleAddData(
        ...
        handleSaleAddDataSuccess,
        handleSaleAddDataException
      );
    } else {
      SaleUpdateData(
        {
  ...
  const handleSaleAddDataSuccess = (dataObject) => {
    setLoading(false);
    sessionStorage.removeItem('orderPlan');
  ...
  const columns = [
    {
      ...
      headerName: (
        <span style={{ fontWeight: "bold", fontSize: "16px" }}>Part Id</span>
      ),
      type: "string",
      sortable: true,
      minWidth: 80,
    ...
    },
    {
      ...
      headerClassName: "super-app-theme--header",
      headerName: (
        <span style={{ fontWeight: "bold", fontSize: "16px" }}>Order QTY</span>
      ),
  ...
  const option2 = PartList.map((item) => ({
    ...
    itemId: item?.itemId ?? "", // fallback to empty string
  }));
  ...
  const AddPartQty = () => {
    // 🔁 Ensure itemId is always correctly linked to partCode
    const selectedPart = option2.find((p) => p.label === partCode);
    const currentItemId = selectedPart?.itemId || itemId; // fallback if not found
  ...
  const handleSaleOrderDataSuccess = (dataObject) => {
    ...
    console.log("globalSalesId>>>>>>>>>>>>>>>>>>>>>>>>>>>", globalSalesId);
  };
  ...
  const handleSaleOrderDataShowSuccess = (dataObject) => {
    ...
    // If sessionStorage has data, use it; otherwise use dataObject
    const finalData = storedData.length > 0 ? storedData : (dataObject?.data || []);
  ...
  const customerNameEnter = (e) => {
    CustomerSearchList(
      ...
      handleCustomerSearchListSuccess,
      handleCustomerSearchListException
    );
  };
  ...
  const SaleTemplateDownload = () => {
    SaleOrderTemplate(
      handleSaleOrderTemplateSuccess,
      handleSaleOrderTemplateException
    );
  };
      ...
      <form className="mt-2 space-y-6" onSubmit={handleSubmit}>
        <DialogContent>
          <Grid container spacing={2}>
              ...
              <FormControl style={{ width: "100%" }}>
                <Autocomplete
                  ...
                  onChange={(event, value) =>
                    handleAutocompleteChangeCustomer(value)
                  }
                />
              </FormControl>
              {/* 
            ...
            <Grid item xs={12} sm={12} md={3} lg={3} xl={3}>
              {/* {user === "Assembly" && <FormControl fullWidth>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        onChange={(e) => setOrderPriority(e.target.checked)}
                      />
                    }
                    label="Urgent"
                  />
              ...
              <FormControl style={{ display: "flex", flexDirection: "column" }}>
                ...
                <div style={{ display: "flex" }}>
                  <FormControlLabel
                    control={
                      <Checkbox
                        checked={isBatchProduction}
                        onChange={(e) => setisBatchProduction(e.target.checked)}
                      />
                    }
            ...
            >
              <Typography>PO | Contract</Typography>
                ...
                >
                  <DataGrid
                    ...
                    getRowClassName={(params) => {
                      ...
                      const rowIndex = partData.findIndex(
                      ...
                      <Button
                        ...
                        onClick={() => {
                          SaleOrderImport(
                            ...
                            handleSaleOrderImportSuccess,
                            handleSaleOrderImportException
                          );
                        }}
                      >
                        Upload
