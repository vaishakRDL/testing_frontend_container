...
const LoadPendingJobwork = ({ mode, setLoadPendingSfg, loadPendingSfg, withoutPOSupplierId, setSelectedItems, supplierSid, selectedItems,onClose ,onSubmit}) => {
    ...
    useEffect(() => {
        loadPendingSfg && LoadPendingJobWorkIssue({ spId: supplierSid }, handleSucessShow, handleExceptionShow)
    }, [loadPendingSfg])
    ...
    function Selector({ selectedRow }) {
        const checked = selectedItems.some((item) => item.id === selectedRow.id);
      
        const handleChange = (e) => {
          if (e.target.checked) {
            const newRow = { 
              ...selectedRow, 
              cumQty: selectedRow.cumQty,   // ✅ use actual cumQty without recalculation
              recievedQty: selectedRow.Qty,
              pendingQty: Number(selectedRow.Qty) - Number(selectedRow.cumQty || 0)
            };
...
export default LoadPendingJobwork;
