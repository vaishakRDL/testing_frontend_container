...
import {
  ...
  NpdGetIdData,
  NpdUpdateData,
  SkillMatrixExlImport,
  SkillMatrixFolderUpload,
  SobUpdate,
} from "../../ApiService/LoginPageService";
import { DataGrid } from "@mui/x-data-grid";
import SearchIcon from "@mui/icons-material/Search";
import {
  DownloadNpdExlTemplate,
  DownloadskillMatrixExlTemplate,
...
const SkillMatrixMultipleFile = ({
  ...
  const handlenPDAddException = (errorObject, errorMessage) => {
    setLoading(false);
    setNotification({
      status: true,
      type: "error",
  ...
  const columns = [
    ...
    },
    {
      ...
      headerClassName: "super-app-theme--header",
      headerName: (
        <span style={{ fontWeight: "bold", fontSize: "16px" }}>CN No</span>
      ),
      type: "string",
      sortable: true,
      minWidth: 100,
    ...
    },
    {
      ...
      headerClassName: "super-app-theme--header",
      headerName: (
        <span style={{ fontWeight: "bold", fontSize: "16px" }}>File Type</span>
      ),
      type: "string",
      sortable: true,
      minWidth: 100,
  ...
  function ViewItem(props) {
    return (
      <RemoveRedEyeIcon style={{ color: "black" }} onClick={(event) => {}} />
    );
  }
  ...
  const NpdExlImportException = (errorObject, errorMessage) => {
    setNotification({
      status: true,
      type: "error",
      message: errorMessage,
  ...
  const handleFolderUpload = async () => {
    ...
    for (const file of selectedFolder) {
      ...
      reader.onload = (event) => {
        ...
        filesData.push({ fileName: file.name, file: base64Data });
        if (filesData.length === selectedFolder.length) {
          ...
          SkillMatrixFolderUpload(
            ...
            NpdFileUploadSuccess,
            NpdFileUploadException
          );
        }
      };
                ...
                <FormControl fullWidth>
                  ...
                  >
                    {sobDataList.map((data) => (
                      <MenuItem key={data.id} value={data.fileType}>
                        {data.fileType}
                      </MenuItem>
                    ))}
                  </Select>
            ...
            <Grid container spacing={2}>
              ...
              >
                <Button
                  ...
                  onClick={(e) => {
                    DownloadskillMatrixExlTemplate(
                      DownloadNpdExlTemplateSuccess,
                      DownloadNpdExlTemplateExption
                    );
                  }}
                >
                  Download Template
...
export default SkillMatrixMultipleFile;
