...
import { MaintanenceCompletedList } from "../../ApiService/LoginPageService";
...
const Completed = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                const res = await new Promise((resolve, reject) => {
                    MaintanenceCompletedList(resolve, reject);
                });
                ...
                const formatted = apiData.map((item, index) => ({
                    id: item?.id ?? index + 1,
                    toolNo: item?.toolNo ?? "-", // safe add
                    Maintype: item?.maintenance_type ?? "-",
                    assetName: item?.machine ?? "-",
    ...
    const columns = useMemo(
        () => [
            ...
            { field: "assetName", headerName: "Machine", flex: 1 },
