...
import { GetMRPPlanningMachine, InfoScrapList, InfoSheetList, OrderOList, SaleOrderFetch } from '../../ApiService/LoginPageService';
import { InfoScrapExport, InfoSheetExport, MrpDownloadorder, MrpPlanningDownloadorder } from '../../ApiService/DownloadCsvReportsService';
...
console.log("selectSalesId--------->>>>>>>>>>>", selectMrpMstId);
    useEffect(() => {
        if (itemDetaildView) {
            OrderOList({
                id: selectSalesId
    ...
    return (
        ...
        >
            <DialogTitle style={{ background: '#002D68', color: 'white', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {
                    isViewData === 0 ? 'Ordered Item List (BOQ)' : isViewData === 1 ? 'Sheet Info' : 'Sheet Scrap Info'
                }
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item sm={12} md={12} lg={12} xl={12}>
                                        <DataGrid
                                            ...
                                            getRowClassName={(params) => {
                                                // Find the index of the row within the rows array
                                                const rowIndex = sheetInfoList.findIndex(row => row.id === params.row.id);
                                                // Check if the index is valid
                                                if (rowIndex !== -1) {
                                                    console.log(' ');
                                                    return rowIndex % 2 === 0 ? 'Mui-evenRow' : 'Mui-oddRow';
                                ...
                                <Grid container spacing={2}>
                                    ...
                                    <Grid item sm={12} md={12} lg={12} xl={12}>
                                        <DataGrid
                                            ...
                                            getRowClassName={(params) => {
                                                // Find the index of the row within the rows array
                                                const rowIndex = sheetScrapList.findIndex(row => row.id === params.row.id);
                                                // Check if the index is valid
                                                if (rowIndex !== -1) {
                                                    console.log(' ');
                                                    return rowIndex % 2 === 0 ? 'Mui-evenRow' : 'Mui-oddRow';
...

                    {
                        isViewData === 0 ? (
                            <>
                                ...
                                <Button
                                    variant="contained"
                                    style={{ width: '220px', background: '#002D68', color: 'white' }}
                                    onClick={(e) => {
                                        MrpPlanningDownloadorder({
                                            id: selectMrpMstId
                                ...
                                <Button
                                    ...
                                    onClick={(e) => {
                                        navigate(`/JobCardViewNewResult?isJobCardStatus=true&&mrpMstId=${selectMrpMstId}`)
