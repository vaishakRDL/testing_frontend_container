...
import './Cancel_Invoice_Report.css';
import { ScrapMstGetMaterial, MachineShowData, ScrapReportAnalysisReport, CustomerDropShowdata, GetSaleInvoiceReport, PartNoSelectShow, CancelSalesInvoice } from "../../ApiService/LoginPageService";
...
const Cancel_Invoice_Report = () => {
    ...
    //     const headers = [
    //         'INV No', 'INV Date', 'Cust Name', 'GST No', 'INV Issue Date',
    ...
    const handleReportDownload = async () => {
        ...
        const headers = [
            'INV No', 'INV Date', 'Cust Name', 'GST No', 'INV Issue Date',
            'INV Qty', 'Item Code', 'Item Name',
            'DC No', 'DC Date', 'Mode Of Desp', 'Vehicle No', 'Cust PONO Date',
            'CESSONTCSPER', 'CESSONTCSAMT', 'Other Amt', 'Tarnsport Amt',
            'TCS Per', 'TCS Amt', 'SURCHGONTCSPER', 'SURCHGONTCSAMT',
            'INSAMT', 'CGSTPER', 'TOtal In Words', 'ASSBL Value',
            'CGST Amt', 'SGST Per', 'AMMCOST', 'SGST Amt',
            'IGST Per', 'IGST Amt', 'UTGST Per', 'UTGST Amt',
            'Less Disc Amt', 'Less Other', 'Amt GST Payable', 'Total Value'
        ];

        worksheet.addRow(headers);

        // Freeze header (optional but normal)
        worksheet.views = [{ state: 'frozen', ySplit: 1 }];
        ...
        reportData.forEach((data) => {
            ...
            data.invDtl.forEach((item, index) => {
                worksheet.addRow([
                    index === 0 ? data.invNo : '',
                    index === 0 ? data.invDate : '',
                    index === 0 ? data.cName : '',
                    index === 0 ? data.gstNo : '',
                    index === 0 ? data.invIssueDate : '',
                    item.invQty,
                    item.itemCode,
                    item.itemName,
                    item.dcNO,
                                ...
                                <table border="1" style={{ borderCollapse: "collapse", width: "100%", zoom: '80%' }} id="customers">
                                    <thead>
                                        <tr>
                                            ...
                                            {/* <th style={{ whiteSpace: 'nowrap' }}>PANDFAMT</th> */}
...
export default Cancel_Invoice_Report;