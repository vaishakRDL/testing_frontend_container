...
import {
    ...
    searchMaterialIssue,
    GetSfgDocNumber,
    SubmitSfgVerification,
    SfgVerificationPreview,
    searchSfgNumber
...
import LoadSfgParts from './LoadSfgParts';
import { useModuleLocks } from '../context/ModuleLockContext';

const NewSfgVarificationResult = ({
    open, setOpen, isAddButton, editeData, setRefreshData,
}) => {

    const { userDetails } = ApplicationStore().getStorage('userDetails');
    const userPermission = userDetails?.groupRights?.filter(
        (data) => data?.menu?.toLowerCase() === "SFGVerification".toLowerCase()
    );

    const moduleLocks = useModuleLocks();
    const isModuleLocked =
        moduleLocks.find(m => m.moduleName === "SFG Verification")?.lockStatus === "locked";
    ...
    const [issuedBy, setIssuedBy] = useState(userDetails?.userName || "");
    ...
    const handleSubmit = (e) => {
        ...
        const payloadData = {
            ...
            sfgNo: docNumber?.uniqueNo,
            sfgLists: selectedItems
        }

        if (isEdit) {
        } else {
            setLoading(true);
            SubmitSfgVerification(payloadData, handleSuccess, handleException);
        }
    };
    ...
    const handleForwardReverse = (type, id) => {
        ...
        SfgVerificationPreview({ type: type, id: id }, handleActionSuccess, handleActionException)
    }
    ...
    const handleActionSuccess = (dataObject) => {
        ...
        const rawDate = data?.sfgDetails?.sfgDate || ''; // Example: "28-02-2025"
        const formattedDate = rawDate ? convertToISO(rawDate) : '';
        setSelectedDate(formattedDate);
        setMainId(data?.sfgDetails?.id || '');
        setDocNumber({ digtit: data?.sfgDetails?.docNo, uniqueNo: data?.sfgDetails?.sfgNo } || { digtit: '', uniqueNo: '' })
        setSelectedItems(dataObject?.sfgItems || []);
        setIssuedBy(data?.sfgDetails?.verifiedBy || '')
    }
    ...
    const handleClearPage = () => {
        ...
        setSelectedItems([]);
        setSearchedIssueNo('');
        GetSfgDocNumber(documentNumberSucess, documentNumberException);
    }
    // setDocNumber
    ...
    const handleSearchChange = (e) => {
        searchSfgNumber({ code: e.target.value }, handleSearchSucessShow, handleSearchExceptionShow);
    }
        ...
        <div>
            <div style={{ display: 'flex', justifyContent: 'flex-start', marginLeft: '10px', marginTop: '10px' }}>
                <Typography
                    ...
                    variant="h5">
                    {isEdit ? "Edit SFG Verification" : isView ? "View SFG Verification" : "New SFG Verification"}
                </Typography>
            </div>
                ...
                <Grid container padding={1}>
                    <Grid item xs={12} sm={12} md={10} lg={10} xl={10}>
                        <Grid container spacing={1}>
                            ...
                            <Grid item xs={12} sm={12} md={2} lg={2} xl={2}>
                                <Button fullWidth variant="contained" style={{ marginTop: '3px', backgroundColor: isView ? "gray" : '#002D68', color: '#ffffff' }} disabled={isView ? true : false} onClick={() => setPendingPOModalOpen(true)}>Load SFG Parts</Button>
                            </Grid>
                        </Grid>
                    </Grid>
                    ...
                    <Grid item xs={12} sm={12} md={2} lg={2} xl={2} style={{ fontSize: '75%' }}>
                        <Autocomplete
                            ...
                            getOptionLabel={(option) => option.sfgNo || searchedIssueNo}
                            renderInput={(params) => <TextField {...params} label="Search SFG" onChange={handleSearchChange} />}
                            ...
                            <CardContent style={{ height: '100%', overflow: 'auto' }}>
                                ...
                                <table id="transactionTable">
                                    <tr style={{ position: 'sticky', top: '-16px', backgroundColor: '#f9f9f9', zIndex: 1 }}>
                                        ...
                                        <th style={{ whiteSpace: 'nowrap' }}>MRP No</th>
                                        <th style={{ whiteSpace: 'nowrap' }}>Kanban Date</th>
                                        <th style={{ whiteSpace: 'nowrap' }}>Part No</th>
                                        <th style={{ whiteSpace: 'nowrap' }}>Part Name</th>
                                        <th style={{ whiteSpace: 'nowrap' }}>Assigned Qty</th>
                                        {isForwardReverse === false &&
                                            <>
                                                <th style={{ whiteSpace: 'nowrap' }}>Produced Qty</th>
                                            </>
                                        }
                                        <th style={{ whiteSpace: 'nowrap' }}>SFG Verified Qty</th>
                                        <th style={{ whiteSpace: 'nowrap' }}>Part Finish</th>
                                        {isForwardReverse === false &&
                                            <>
                                                <th style={{ whiteSpace: 'nowrap' }}>Vendor Process</th>
                                                <th style={{ whiteSpace: 'nowrap' }}>Next Process Name</th>
                                    ...
                                    {selectedItems.map((item, index) => {
                                        return (
                                            <tr key={index}>
                                                {/* <td contentEditable={false}>
                                                    {item.itemCode ? <span>{item.itemCode}</span> :
                                                        <Autocomplete
                                                            ...
                                                            getOptionLabel={(option) => `𝐈𝐭𝐞𝐦 𝐂𝐨𝐝𝐞: ${option?.itemCode} | 𝐒𝐮𝐩𝐩 𝐃𝐞𝐬𝐜: ${option.suppDesc} | 𝐏𝐎: ${option.poNo} | 𝐒𝐜𝐡 𝐃𝐚𝐭𝐞: ${option.schDate} | 𝐐𝐭𝐲: ${option.poQty} | 𝐏𝐎 𝐑𝐚𝐭𝐞: ${option.pbRate}` || ''}
                                                            sx={{ width: 300 }}
                                                ...
                                                <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.jcNo}</td>
                                                <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.mrpNo}</td>
                                                <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.kanbanDate}</td>
                                                ...
                                                <td style={{ whiteSpace: 'nowrap' }} contentEditable={false} >{item.Qty}</td>
                                                {isForwardReverse === false &&
                                                    <>
                                                        <td style={{ whiteSpace: 'nowrap' }} contentEditable={false} >{item.producedQty}</td>
                                                    </>
                                                }
                                                <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.sfgVerifiedQty}</td>
                                                <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.productFinish}</td>
                                                {isForwardReverse === false &&
                                                    <>
                                                        <td style={{ whiteSpace: 'nowrap' }} contentEditable={false} onBlur={(e) => handleEdit('cumQty', e.target.textContent, item.id, item)}>{item.vendorProcess}</td>
                                                        <td style={{ whiteSpace: 'nowrap' }} contentEditable={false} >{item.nextProcess}</td>
                                                        <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.machineName}</td>
                                                    </>
                                                }
...
export default NewSfgVarificationResult