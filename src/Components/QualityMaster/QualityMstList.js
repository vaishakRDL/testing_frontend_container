...
import QualityMstTools from './QualityMstTools';
import { QualityInsMstDelete, QualityInsMstShowData } from '../../ApiService/LoginPageService';
import QualityMstModel from './QualityMstModel';
import ApplicationStore from '../../Utility/localStorageUtil';
import { useModuleLocks } from '../context/ModuleLockContext';

const QualityMstList = () => {
  ...
  const isModuleLocked =
    moduleLocks.find(m => m.moduleName === "Quality Master")?.lockStatus === "locked";
        ...
        <EditIcon
          ...
          onClick={(event) => {
            if (userPermission[0]?.updateData === 1 && !isModuleLocked) {
              ...
              setEditQuality(props.selectedRow);
              setOpen(true);
            }
          }}
        />
  ...
  const DeleteFunction = () => {
    QualityInsMstDelete(handleDeleteSuccess, handleDeleteException);
  }
