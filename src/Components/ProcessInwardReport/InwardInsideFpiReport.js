...
import { GetItemVsProcessItem, GetItemVsProcessProcessList, AddItemVsProcess, ProcessInspecReportViewId, QcInwardReportViewId } from '../../ApiService/LoginPageService';
import { useNavigate } from 'react-router-dom';
import { QualityReport } from '../../ApiService/DownloadCsvReportsService';
import InwardInsideFpiTitle from './InwardInsideFpiTitle';
...
const InwardInsideFpiReport = ({ setIsFpiReport, slNO, reportType, jcId, itemId, inwordReport, selectedRowId }) => {
    ...
    //     }),
    //     `FPI_Report_${new Date().toLocaleDateString()}.xlsx`
    //   );
    // };
    const handleExcelDownload = async () => {
        if (!dataset || dataset.length === 0 || columns2.length === 0) {
        ...
        // -----------------------------------------
        // 🔹 Reserve space for logo (Avoid overlap)
        for (let i = 0; i < 5; i++) {
            worksheet.addRow([]);
        }
        ...
        if (companyLogoPath) {
            ...
            worksheet.addImage(imageId, {
                tl: { col: 0, row: 0 }, // positioned top-left corner
        ...
        dataset.forEach(row => {
            worksheet.addRow(columns2.map(col => toNumberIfPossible(row[col.field])));
        });
        ...
        headerRow.eachCell((cell) => {
            ...
            cell.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "78bce9ff" } };
            cell.alignment = { horizontal: "center", vertical: "middle" };
        ...
        worksheet.columns.forEach(col => {
            let max = 10;
            col.eachCell(cell => {
                max = Math.max(max, cell.value ? String(cell.value).length : 0);
            });
            col.width = Math.min(max + 3, 35);
        });
        ...
        saveAs(
            ...
            `Against PO QC Inward Report_${new Date().toLocaleDateString()}.xlsx`
        );
    };
...
export default InwardInsideFpiReport;