...
import InwardInsideFpiReport from "./InwardInsideFpiReport";
...
const FPIProcessInwardReport = ({ reportType }) => {
  ...
  const AgainstExcelImportSuccess = (dataObject) => {
    InwardInspecReport({
      ...
      from: fromDate,
      to: toDate
    }, handleProcessInspecReportSuccess, handleProcessInspecReportException); setNotification({
      status: true,
      type: 'success',
    ...
    },
    {
      ...
      renderCell: (params) => (
        <div style={{ display: "flex", justifyContent: "space-around", alignItems: 'center' }}>
          ...
          {/* <IconButton
            onClick={() => {
              const fileUrl = params.row.qcFile;
              if (fileUrl) {
                setSelectedFilePath(fileUrl);
...
export default FPIProcessInwardReport;
