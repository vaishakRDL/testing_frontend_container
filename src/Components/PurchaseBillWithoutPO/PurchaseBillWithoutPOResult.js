...
import PurchaseBillWithoutPOTitle from './PurchaseBillWithoutPOTitle';
import RemoveRedEyeIcon from '@mui/icons-material/RemoveRedEye';
import { ShowForeCastEntry, DeletePurchaseBillWithoutPO, ViewWithoutPoList } from '../../ApiService/LoginPageService'
import DeleteConfirmationDailog from '../../Utility/confirmDeletion';
import { Link, useNavigate } from 'react-router-dom';
import '../../App.css';
import ApplicationStore from '../../Utility/localStorageUtil';

const PurchaseBillWithoutPOResult = (props) => {
    ...
    const [withoutPoLists, setWithoutPoLists] = useState([]);
    ...
    useEffect(() => {
        ViewWithoutPoList(handleSucessShow, handleExceptionShow)
    }, [refreshData]);

    const handleSucessShow = (dataObject) => {
        setWithoutPoLists(dataObject?.data || []);
    }
    ...
    function ViewData(props) {
        return (
            <RemoveRedEyeIcon
                style={{ color: '#000000' }}
                onClick={() => {
                    navigate(`/PurchaseBillWithoutPOModule?isView=true&&PBNo=${props.selectedRow.digit}&&qcApproval=${props.selectedRow.qcApproval}`)
                }}
            />
        );
    }
            ...
            <EditIcon
                ...
                onClick={() => {
                    if (userPermission[0]?.updateData === 1) {
                        navigate(`/PurchaseBillWithoutPOModule?isEdit=true&&PBNo=${props.selectedRow.digit}&&qcApproval=${props.selectedRow.qcApproval}`)
                    }
                }}
            />
        );
...
export default PurchaseBillWithoutPOResult