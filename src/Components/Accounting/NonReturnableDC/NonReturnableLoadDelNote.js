...
const NonReturnableLoadDelNote = ({ loadPendingModalOpen, setLoadPendingModalOpen, setPendingPOList, customerId }) => {
    ...
    const handlesubmit = () => {
        GetPendingDelNoteData({ cusId: customerId, delNo: selectedDelNote }, handleGetDataSuccess, handleGetDataException);
    }
                ...
                <form className="mt-2 space-y-6">
                    <DialogContent>
                        <Grid container spacing={2}>
                            <Grid item sm={12} md={3} lg={3} xl={3}>
                                <TextField
                                    ...
                                    onChange={(e) => {
                                        ...
                                        toDate && SearchDelNote({ id: customerId, form: e.target.value, to: toDate, isWarehouse: 1 }, handleLoadPendingDelNote, handeLoadPendingDelNoteException);
                            ...
                            <Grid item sm={12} md={3} lg={3} xl={3}>
                                <TextField
                                    ...
                                    onChange={(e) => {
                                        ...
                                        formDate && SearchDelNote({ id: customerId, form: formDate, to: e.target.value, isWarehouse: 1 }, handleLoadPendingDelNote, handeLoadPendingDelNoteException);
                                        // selectedDelNote && GetPendingDelNoteData({ cusId: customerId, delNo: selectedDelNote }, handleGetDataSuccess, handleGetDataException)
                            ...
                            {/* <Grid item sm={12} md={3} lg={3} xl={3}>
                                <FormControl fullWidth>
                                    ...
                                    <Select
                                        ...
                                        value={location === "" ? "" : location === 0 ? "Customer_Location" : "Warehouse"}
                                        variant="filled"
                                        label="Del Note"
                                        onChange={(e) => {
                                            const selectedValue = e.target.value === "Customer_Location" ? 0 : 1;
                                            setLocation(selectedValue);
                                            toDate &&  formDate && SearchDelNote({ id: customerId, form: formDate, to: toDate ,isWarehouse : selectedValue }, handleLoadPendingDelNote, handeLoadPendingDelNoteException);
                                            selectedDelNote && GetPendingDelNoteData({ cusId: customerId, delNo: selectedDelNote }, handleGetDataSuccess, handleGetDataException)
                                        }}
                                    >
                            ...
                            <Grid item sm={12} md={3} lg={3} xl={3}>
                                <FormControl fullWidth>
                                    <InputLabel id="demo-multiple-select-label">Del Note</InputLabel>
                                    <Select
                                    ...
                                    >
                                        {delNoteLists.map((data) => (
                                            <MenuItem key={data.delNoteNo} value={data.delNoteNo}>
                                                <Checkbox checked={selectedDelNote.indexOf(data.delNoteNo) > -1} />
                                                {data.delNoteNo}
                                            </MenuItem>
                                        ))}
                                    </Select>
                            ...
                            <Grid item sm={12} md={3} lg={3} xl={3}>
                                <Button variant="contained" style={{ height: '50px', backgroundColor: '#002d68', width: "100px" }} onClick={handlesubmit}>
                                    View
                                </Button>
                            </Grid>
...
export default NonReturnableLoadDelNote
