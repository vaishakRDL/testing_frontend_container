...
import { ScrapMstGetMaterial, MachineShowData, ScrapReportAnalysisReport, CustomerDropShowdata, PartNoSelectShow, GetItemwiseReport } from "../../../../ApiService/LoginPageService";
import NotificationBar from "../../../GlobleFiles/ServiceNotificationBar";
import { SalesInvoiceItemWise, ScrapAnalysisReportDownload } from "../../../../ApiService/DownloadCsvReportsService";
...
const ItemwiseReport = () => {
    ...
    const handleReportExcelDownload = async () => {
        if (loader) return;
        setLoader(true);
        try {
            const payload = {
                from: fromDate,
                to: toDate,
                item: Array.isArray(partNo) ? partNo : partNo ? [partNo] : [],
                customer: Array.isArray(customerSelect) ? customerSelect : customerSelect ? [customerSelect] : []
            };
            // console.log('=== Export Payload ===', JSON.stringify(payload)); // ← check this
            SalesInvoiceItemWise(
                payload,
                (response) => {
    ...
    const filteredReportData = React.useMemo(() => {
        return reportData.filter(d => d.invNo !== "Grand Total");
    }, [reportData]);
                ...
                <CardContent>
                    <Grid container alignItems={'center'} spacing={2}>
                        ...
                        <Grid item xs={12} sm={8} md={3} lg={3}>
                            ...
                            <Button variant="contained" style={{ backgroundColor: '#002D68', marginTop: '2px' }} onClick={handleReportExcelDownload}>Download</Button>
                        </Grid>
                                ...
                                <table border="1" style={{ borderCollapse: "collapse", width: "100%", zoom: '80%' }} id="customers">
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ whiteSpace: 'nowrap' }}>Tarrif Name</th>
                                            <th style={{ whiteSpace: 'nowrap' }}>UOM Code</th>
                                            {/* <th style={{ whiteSpace: 'nowrap' }}>Vehicle No</th> */}
                                            <th style={{ whiteSpace: 'nowrap' }}>INV Qty</th>
                                            <th style={{ whiteSpace: 'nowrap' }}>INV Rate</th>
                                            <th style={{ whiteSpace: 'nowrap' }}>Net Amt</th>
                                            ...
                                            <th style={{ whiteSpace: 'nowrap' }}>Gross Value</th>

                                        </tr>
                                    </thead>
                                    <tbody>
                                        {filteredReportData.map((data) => {
                                            const rowSpan = data.invDtl?.length || 0;

                                            return data.invDtl.map((item, index) => (
                                                <tr key={`${data.invNo}-${item.itemCode}-${index}`}>
                                                    {index === 0 && (
                                                        <>
                                                            <td rowSpan={rowSpan}>{data.invNo}</td>
                                                            <td rowSpan={rowSpan}>{data.invDate}</td>
                                                            <td rowSpan={rowSpan}>{data.cCode}</td>
                                                            <td rowSpan={rowSpan}>{data.cName}</td>
                                                            <td rowSpan={rowSpan}>{data.gstNo}</td>
                                                            <td rowSpan={rowSpan}>{data.vechileNO}</td>
                                                        </>
                                                    )}
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td colSpan={23} style={{ textAlign: 'left', fontWeight: 'bold' }}>Grand Total</td>
                                            <td style={{ fontWeight: 'bold' }}>
                                                {
                                                    reportData.find(d => d.invNo === "Grand Total")?.totalValue || "0.00"
                                                }
                                            </td>
                                        </tr>
                                    </tfoot>
