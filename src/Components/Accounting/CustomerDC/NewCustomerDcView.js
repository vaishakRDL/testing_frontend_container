...
import { ExportCustomerDC } from "../../../ApiService/DownloadCsvReportsService";
...
const NewPurchaseOderEntryView = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerClassName: "super-app-theme--header",
      headerName: (
        <span style={{ fontWeight: "bold", fontSize: "16px" }}>NRDC Date</span>
      ),
      width: 70,
      type: "string",
      sortable: true,
    ...
    return (
      <Tooltip disableFocusListener title="Edit">
        <EditIcon
          ...
          onClick={() => {
            navigate(
              `/NewCustomerDc?isEdit=true&&poRowId=${props.selectedRow}`
            );
          }}
        />
      </Tooltip>
  ...
  useEffect(() => {
    CustomerDcShowData(handleCustomerDcShow, handeCustomerDcException);
  }, [refreshData]);

  const handleCustomerDcShow = (dataObject) => {
    setRows(dataObject?.data || []);
  };

  const handeCustomerDcException = (errorStatus, errorMessage) => {
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            <Link to="/NewCustomerDc" style={{ textDecoration: "none" }}>
              <Typography
