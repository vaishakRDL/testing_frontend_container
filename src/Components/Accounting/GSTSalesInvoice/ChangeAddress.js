...
const ChangeAddress = ({
  ...
  const [selctedrow, setSelectedRows] = useState("");
  ...
  //   const selectedId = selectionModel[0]; // Only one selection expected
  //   const selectedRow = custAllAddressList.find((row) => row.id === selectedId);

  //   if (selectedRow) {
  //     // Combine fields or use the format you want
  //     const newAddress = `${selectedRow.address || ""}`;

  //     setBillingAddress(newAddress); // This updates parent billing address field
  //     setChangeAddressModalOpen(false); // Close dialog
  //   }
  // };
  const handleDatagridRowSelection = (selectionModel) => {
                ...
                <CardContent>
                  <DataGrid
                    ...
                    getRowClassName={(params) => {
                      const rowIndex = custAllAddressList.findIndex(
