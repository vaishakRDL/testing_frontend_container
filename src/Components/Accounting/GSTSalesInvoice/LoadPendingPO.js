...
const LoadPendingPO = ({ setVechicleNo, loadPendingModalOpen, invType, setLoadPendingModalOpen, setPendingPOList, customerId }) => {
    ...
    const handleDeleteRow = (id) => {
        const newArray = rows1.filter((item) => item.id != id)
        setSelectedRows(newArray);
    }

    useEffect(() => {
        loadPendingModalOpen && GSTLoadPendingPO({ id: customerId, type: invType }, handleLoadPendingPOShow, handeLoadPendingPOException);
