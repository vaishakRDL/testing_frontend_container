import {
    Button, Box, Dialog, Tooltip, DialogContent, CircularProgress, DialogTitle, TextField, Grid, DialogActions, InputLabel, Select, MenuItem, FormControl, FormControlLabel, Checkbox, CardContent, Typography, Autocomplete, InputAdornment, FormLabel, RadioGroup, Radio, Card,
    FormGroup,
} from '@mui/material';
...
import { AssemblySaleOrderImport, CustomerSearch, CustomerSearchList, GetAssemblyPlanningFIM, ItemSearchNAAJ, ItemfetchItemsSearch, SaleAddData, SaleGetId, SaleGetOrderNo, SaleOrderData, SaleOrderDataDelete, SaleOrderDataShow, SaleOrderDelete, SaleOrderImport, SaleOrderUpdateData, SaleScrapRejectedItems, SaleUpdateData } from '../../ApiService/LoginPageService';
import { AssemblySaleOrderTemplate, SaleOrderTemplate } from '../../ApiService/DownloadCsvReportsService';
import { useLocation, useNavigate } from 'react-router-dom';
import ApplicationStore from '../../Utility/localStorageUtil';

const SalesModule = ({
    open, setOpen, isAddButton, editeData, setRefreshData, user, isRejected, selectedItems
}) => {
    ...
    const [generateSrn, setGenerateSrn] = useState(true);
    const [referenceNumber, setReferenceRember] = useState('');
    const navigate = useNavigate();
    const location = useLocation();

    const AssemblySalesResult = new URLSearchParams(location.search).get('AssemblySalesResult');
    ...
    const handlerDataShowSuccess = (dataObject) => {
        setPartData(dataObject?.data || []);
    }

    const handleDataShowException = (dataObject) => {

    }

    const LoadData = () => {
        setCustomerName(editeData?.cName || '');
    ...
    const handleSaleGetIdSuccess = (dataObject) => {
        ...
        setSaleOrderId(dataObject?.id || '');
        // setSaleOrderId()
        if (isRejected) {
            SaleScrapRejectedItems({ saleId: dataObject?.id, scrapItems: selectedItems }, handleScrapItemSuccess, handleScrapItemException)
        }
    }

    const handleScrapItemSuccess = (dataObject) => {
        SaleOrderDataShow({
            salesId: dataObject?.saleId || ''
        }, handleSaleOrderDataShowSuccess, handleSaleOrderDataShowException);
    }
    const handleScrapItemException = () => { }
        ...
        return (
            <EditIcon
                ...
                onClick={(event) => {
                    ...
                    setPartName(props.selectedRow?.itemName || '')
    ...
    function DeleteData(props) {
        const handleDeleteItem = (id) => {
            const updatedPartData = partData.filter((item) => item.id !== id);
        ...
        return (
            <DeleteIcon
                ...
                // }}
                onClick={() => handleDeleteItem(props.selectedRow.id)}
                style={{ color: 'black' }}
            />
        );
    }
    ...
    const AddPartQty = () => {
        if (inAdd) {
            setPartData([...partData, { id: Part, itemCode: partCode, itemName: PartName, Qty: qty, remarks: remarks2, refNo: referenceNumber, itemId: itemId }])
            setPart('');
            setPartCode('');
            setPartName('');
            setQty('');
            ...
            const updatedPartData = partData.map(part =>
                part.id === inId
                    ? { ...part, itemCode: partCode, itemName: PartName, Qty: qty, remarks: remarks2, refNo: referenceNumber, itemId: itemId }
                    : part
            );
            setPartData(updatedPartData);
            setPart('');
    ...
    const SaleTemplateDownload = () => {
        AssemblySaleOrderTemplate(handleSaleOrderTemplateSuccess, handleSaleOrderTemplateException);
    }
    ...
    return (
        ...
        >
            <DialogTitle style={{ background: '#002D68', color: 'white', display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                {isAddButton ? 'Add Assembly Order' : 'Edit Assembly Order'}
            ...
            <form className="mt-2 space-y-6" onSubmit={handleSubmit}>
                <DialogContent>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={12} md={3} lg={3} xl={3}>
                            <FormControl fullWidth>
                                ...
                                <Select
                                    labelId="demo-simple-select-label"
                                    id="demo-simple-select"
                                    label="Select Assembly Cell"
                                    placeholder='Select Assembly Cell'
                                    variant="filled"
                                    size='small'
                                    value={selectedFIM}
                                    onChange={(e) => {
                        ...
                        >
                            <FormGroup>
                                <FormControlLabel control={<Checkbox checked={generateSrn} onChange={(e) => setGenerateSrn(e.target.checked)} />} label="Generate SRN" />
                            </FormGroup>
                        </Grid>
                                    ...
                                    <>
                                        ...
                                        <Grid item xs={12} sm={12} md={12} lg={12} xl={12}>
                                            <TextField
                                                ...
                                                onChange={(e) => setReferenceRember(e.target.value)}
                ...
                <DialogActions>
                    ...
                    >
                        {viewloading ? (
                            ...
                            isAddButton ? 'Add Assembly Orders Input' : 'Update Assembly Orders Input'
                        )}
                    </Button>
                    <Button
                        variant="contained"
