...
import BomModuleTitle from './BomModuleTitle';
import { BomDeleteD, BomFetchId, BomItemsDelete, BomItemsShow, BomItemsUpdate, BomItemRowInsert, UpdateNewBomEntry } from '../../ApiService/LoginPageService';
import NotificationBar from '../GlobleFiles/ServiceNotificationBar';
import DeleteIcon from '@mui/icons-material/DeleteOutlined';
import DeleteConfirmationDailog from '../../Utility/confirmDeletion';
import DeleteConfirmationDailogBOM from '../../Utility/confirmDeletionBom';
import SearchIcon from '@mui/icons-material/Search';
import AllBOMList from './AllBOMList';
import ApplicationStore from '../../Utility/localStorageUtil';
import { DownloadAllBomXl } from '../../ApiService/DownloadCsvReportsService';
...
const RecursiveTreeView = ({ data }) => (
    <TreeView defaultCollapseIcon={<MinusSquare />} defaultExpandIcon={<PlusSquare />} defaultEndIcon={<CloseSquare />}>
        {data.map((item) => (
            <StyledTreeItem key={item.id} nodeId={item.id} label={item.showLabel || item.label}>
...
const BomModule = () => {
    ...
    const isModuleLocked =
        moduleLocks.find(m => m.moduleName === "BOM")?.lockStatus === "locked";

    const label = { inputProps: { 'aria-label': 'Checkbox demo' } };
    const [open, setOpen] = useState(false);
    const [mainId, setMainId] = useState('');
    console.log("(dataObject?.bom,,,,,,", mainId)
    ...
    const handleBomItemsShowSuccess = (dataObject) => {
        // console.log("dataObject?.data ", dataObject?.data[0].itemId)
    ...
    const handleBomFetchIdSuccess = (dataObject) => {
        ...
        setMainId(dataObject?.bomTree[0]?.id || []);
    ...
    const onTextSearch = (e) => {
        BomItemsShow({
            text: e.target.value
        }, handleBomItemsShowSuccess, handleBomItemsShowException);
    }
    ...
    const handleEdit = (cellNam, newValue, id, rowData) => {
        switch (cellNam) {
            case "Qty":
                const updatedQty = selectedItems.map((supp) =>
                    supp.id === id && cellNam === 'Qty' ?
                        { ...supp, Qty: newValue }
                        : supp
                )
                setSelectedItems(updatedQty);
                break;
            default:
        }
    }
    ...
    const handleSaveData = () => {
        ...
        UpdateNewBomEntry({
            ...
            bomList: updatedArray
        }, handleBomUpdateSuccess, handleBomUpdateException)
    }

    const handleBomUpdateSuccess = (dataObject) => {
    ...
    const handleBomUpdateException = (errorObject, errorMessage) => {
        setsaveLoading(false)
                                ...
                                <table id="customers">
                                    ...
                                    {selectedItems.map((item, index) => (
                                        <tr key={index} onClick={() => handleTableRowClick(index)}>
                                            ...
                                            <td style={{ whiteSpace: 'nowrap' }} contentEditable={false}>{item.uomName}</td>
                                            <td style={{ whiteSpace: 'nowrap' }} contentEditable={false} /*onBlur={(e) => handleEdit('srnQty', e.target.textContent, item.id, item)}*/>
                                                {/* {item.isBom} */}
                                                <Checkbox
                                                    checked={item.isBom === 'Y' ? true : false}
                                                    // onChange={(e) => isChangeBom(e.target.checked, item.id)}
                                                    inputProps={{ 'aria-label': 'controlled' }}
                                                />
                                            </td>
                                            <td style={{ whiteSpace: 'nowrap' }} contentEditable={isEditRow} onBlur={(e) => handleEdit('Qty', e.target.textContent, item.id, item)}>{item.Qty}</td>
                                            <td style={{ whiteSpace: 'nowrap' }} contentEditable={isEditRow}>
                                                <select
                                                    value={item.jcPart}
                                                    onChange={(e) => handleDropdownChange(e.target.value, item.id)}
                                                    style={{ width: '100%', border: 'none', background: 'transparent' }}
                                                    disabled={isEditRow == true ? false : true}
                                    ...
                                    >
                                        <Tooltip title="Add">
                                            <Button variant="contained"
                                                style={{ width: '50px', background: userPermission[0]?.addData === 0 || isModuleLocked ? 'gray' : '#002D68', color: userPermission[0]?.addData === 0 || isModuleLocked ? 'black' : 'white' }}
                                        ...
                                        <Tooltip title="Edit">
                                            <Button variant="contained"
                                                style={{ width: '50px', background: userPermission[0]?.updateData === 0 || isModuleLocked ? 'gray' : '#002D68', color: userPermission[0]?.updateData === 0 || isModuleLocked ? 'black' : 'white' }}
                                        ...
                                        <Tooltip title="Delete ">
                                            <Button variant="contained"
                                                style={{ width: '50px', background: userPermission[0]?.deleteData === 0 || isModuleLocked ? 'gray' : '#002D68', color: userPermission[0]?.deleteData === 0 || isModuleLocked ? 'black' : 'white' }}
                                        ...
                                        <Tooltip title="Download">
                                            <Button variant="contained"
                                                ...
                                                disabled={downloadloading} onClick={() => {
                                                    ...
                                                    DownloadAllBomXl({ id: mainId }, handleExportSuccess, handleExportException)
                                                }}
                                            >
                                                {downloadloading ? (
                                        ...
                                        <Tooltip title="Save">
                                            <Button variant="contained"
                                                style={{ width: '68px', background: userPermission[0]?.deleteData === 0 || isModuleLocked ? 'gray' : '#002D68', color: userPermission[0]?.deleteData === 0 || isModuleLocked ? 'black' : 'white' }}
                                                disabled={userPermission[0]?.deleteData === 0 || isModuleLocked || saveloading}
                                                // onClick={handleAddRowDelete}
